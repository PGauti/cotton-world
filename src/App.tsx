...
function CanvasApp() {
  ...
  const getNodeHeight = (type) => {
    ...
    return 240; // Action nodes with link field
  };
    ...
    <div className="flex h-screen bg-black text-gray-100 font-sans overflow-hidden select-none" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}>
      ...
      <aside className="w-72 bg-[#1c1c1e]/90 backdrop-blur-2xl border-r border-white/10 flex flex-col shrink-0 z-50 shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
          {journeysList.map((j) => (
            <div key={j.id} onClick={() => setActiveJId(j.id)} className={`w-full text-left px-4 py-2.5 rounded-xl text-[11px] font-bold transition-all flex items-center justify-between group cursor-pointer ${activeJId === j.id ? 'bg-[#0A84FF] text-white' : 'text-gray-500 hover:bg-white/5'}`}>
        ...
        <div className="p-4 border-t border-white/10">
           {showAuthInput && !isAdmin && (
             <div className="mb-3 flex gap-2">
               <input type="password" placeholder="Passcode..." className="flex-1 bg-black border border-white/10 rounded-lg px-3 py-2 text-[11px] text-white outline-none" value={authInput} onChange={e => setAuthInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && authInput === 'admin2024' && (setIsAdmin(true), setShowAuthInput(false))} />
             </div>
           )}
        ...
        <div className="w-full h-full relative" style={{ backgroundImage: 'radial-gradient(#1c1c1e 1.5px, transparent 1.5px)', backgroundSize: '60px 60px' }}>
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none">
            {edges.map(e => {
              ...
              const x1 = from.x, y1 = from.y + (h1 / 2);
              const x2 = to.x, y2 = to.y - (getNodeHeight(to.type) / 2);
              const color = e.port === 'true' ? '#32D74B' : e.port === 'false' ? '#FF453A' : '#5E5E62';

              return (
                <g key={e.id}>
                  <path d={calculatePath(x1, y1, x2, y2)} stroke={color} strokeWidth="3" fill="none" opacity="0.3" />
            ...
            {linking && (
               <path d={calculatePath(nodes.find((n:any) => n.id === linking.fromId).x, nodes.find((n:any) => n.id === linking.fromId).y + (getNodeHeight(nodes.find((n:any) => n.id === linking.fromId).type)/2), mousePos.x, mousePos.y)} stroke="#0A84FF" strokeWidth="2" strokeDasharray="6,6" fill="none" />
            )}
          </svg>
          ...
          {nodes.map((n: any) => (
            <div key={n.id} className="absolute transform -translate-x-1/2 -translate-y-1/2" style={{ left: n.x, top: n.y }}>
              ...
              {linking && linking.fromId !== n.id && (
                <div onMouseUp={(e) => completeLinking(e, n.id)} className="absolute inset-0 z-[100] bg-[#0A84FF]/10 border-2 border-[#0A84FF] border-dashed rounded-3xl animate-pulse cursor-crosshair" style={{ width: 260, height: getNodeHeight(n.type) }} />
              )}
              
              <div 
                className={`w-[260px] bg-[#1c1c1e]/90 backdrop-blur-xl rounded-3xl border transition-all duration-300 ${dragNode === n.id ? 'scale-105 border-[#0A84FF] shadow-[0_0_50px_rgba(10,132,255,0.2)] z-50' : 'hover:border-white/20 border-white/5 shadow-2xl z-20'}`}
              >
                <div onMouseDown={(e) => { const rect = e.currentTarget.closest('div')!.getBoundingClientRect(); setDragOffset({ x: e.clientX - rect.left, y: e.clientY - rect.top }); setDragNode(n.id); }} className="px-5 py-2.5 border-b border-white/5 flex justify-between items-center bg-white/5 cursor-grab rounded-t-3xl">
                  ...
                  <div className="flex gap-1 items-center">
                    ...
                    <button className="p-1 text-gray-600 hover:text-red-500" onClick={() => { setNodeData((p:any) => ({...p, [activeJId]: p[activeJId].filter((it:any) => it.id !== n.id)})); setEdgeData((p:any) => ({...p, [activeJId]: p[activeJId].filter((it:any) => it.from !== n.id && it.to !== n.id)})); }}><Trash2 size={11} /></button>
                  </div>
                </div>

                <div className="p-5 space-y-3" onMouseDown={e => e.stopPropagation()}>
                   <input className="bg-transparent text-sm font-bold outline-none w-full border-b border-transparent focus:border-white/10" value={n.label || n.title} onChange={e => { const upd = { label: e.target.value, title: e.target.value }; setNodeData((p:any) => ({...p, [activeJId]: p[activeJId].map((it:any)=>it.id===n.id?{...it,...upd}:it)})); }} onBlur={() => syncToCloud(journeysList, nodeData, edgeData)} />
                   
                   {n.type === 'action' && (
                     <>
                        <textarea className="w-full bg-black/40 p-3 rounded-2xl text-[11px] h-18 outline-none resize-none leading-relaxed text-gray-400 italic border border-white/5 focus:border-[#0A84FF]/30 transition-all" value={n.content} placeholder="Message content..." onChange={e => { setNodeData((p:any)=>({...p,[activeJId]:p[activeJId].map((it:any)=>it.id===n.id?{...it,content:e.target.value}:it)})); }} onBlur={() => syncToCloud(journeysList, nodeData, edgeData)} />
                        
                        {/* THE PREVIEW / MEDIA URL FIELD */}
                        <div className="flex flex-col gap-1 mt-1">
                           ...
                           <div className="flex items-center bg-black/40 border border-white/5 rounded-xl overflow-hidden focus-within:border-[#0A84FF]/40 transition-colors">
                              ...
                              <input 
                                ...
                                value={n.previewLink || ''} 
                                onChange={e => { setNodeData((p:any)=>({...p,[activeJId]:p[activeJId].map((it:any)=>it.id===n.id?{...it,previewLink:e.target.value}:it)})); }} 
                                onBlur={() => syncToCloud(journeysList, nodeData, edgeData)}
                              />
                           </div>
                           {n.previewLink && n.previewLink.trim() !== '' && (
                              <a href={n.previewLink.startsWith('http') ? n.previewLink : `https://${n.previewLink}`} target="_blank" rel="noopener noreferrer" className="mt-1 w-full bg-[#0A84FF]/80 hover:bg-[#0A84FF] text-white py-1.5 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-1.5 transition-all shadow-lg">
                                Test / Preview ↗
                              </a>
                           )}
                        </div>
